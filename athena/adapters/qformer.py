"""
Q-Former adapter implementation for BLIP-2 models.
"""
...
class QFormerAdapter(nn.Module):
    """
    Q-Former adapter that extends the base PolyAdapter for BLIP-2 models.
    
    Args:
        query_features: Query token feature dimension
    ...
    def __init__(
        ...
        self.query_embeddings = nn.Parameter(
            torch.randn(1, num_query_tokens, query_features)
        )
        ...
        # Query adapter components
        if use_lora:
            self.query_lora = nn.ModuleDict({
                'q': nn.Linear(query_features, rank, bias=False),
                'k': nn.Linear(query_features, rank, bias=False),
                'v': nn.Linear(query_features, rank, bias=False),
                'o': nn.Linear(rank, query_features, bias=False)
            })
        
        if use_ia3:
            self.query_ia3 = nn.ParameterDict({
                'q': nn.Parameter(torch.ones(query_features)),
                'k': nn.Parameter(torch.ones(query_features)),
                'v': nn.Parameter(torch.ones(query_features))
            })
        
        if use_moe:
            self.query_moe = nn.ModuleList([
                nn.Sequential(
                    nn.Linear(query_features, query_features),
                    nn.GELU(),
                    nn.Linear(query_features, query_features)
                ) for _ in range(num_experts)
            ])
            self.query_router = nn.Linear(query_features, num_experts)
    ...
    def forward(
        ...
        Returns:
            Tuple of (query features, updated vision features, updated text features)
        """
        batch_size = vision_features.size(0)
        
        # Expand query embeddings to batch size
        query_features = self.query_embeddings.expand(batch_size, -1, -1)
        
        # Apply cross-attention if enabled
        if self.use_cross_attention:
            # Vision cross-attention
            vision_attended, _ = self.vision_cross_attention(
            ...
            text_attended, _ = self.text_cross_attention(
                query=query_features,
                key=text_features,
                value=text_features,
                key_padding_mask=attention_mask
            ...
            query_features = vision_attended + text_attended
        else:
            vision_attended = vision_features
            text_attended = text_features
        
        # Apply query adapters
        query_output = query_features
        if self.use_lora:
            query_output = query_output + self._apply_lora(
                query_features,
                self.query_lora,
                self.query_ia3 if self.use_ia3 else None
            )
        if self.use_moe:
            query_output = query_output + self._apply_moe(
                query_features,
                self.query_moe,
                self.query_router,
                self.k
        ...
        return query_output, vision_output, text_output
    
    def get_trainable_params(self) -> Dict[str, torch.Tensor]:
        """Get dictionary of trainable parameters."""
        params = {}
        
        params['query_embeddings'] = self.query_embeddings
        
        if self.use_cross_attention:
            params.update({
                'vision_cross_attention': self.vision_cross_attention.state_dict(),
                'text_cross_attention': self.text_cross_attention.state_dict()
            })
        
        if self.use_lora:
            params.update({
                'query_lora': self.query_lora.state_dict(),
        ...
        if self.use_ia3:
            params.update({
                'query_ia3': self.query_ia3.state_dict(),
        ...
        if self.use_moe:
            params.update({
                'query_moe': self.query_moe.state_dict(),
                'query_router': self.query_router.state_dict(),
